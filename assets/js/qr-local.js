/*
 * Gerador QR local.
 * Baseado no QRCode for JavaScript, de Kazuhiko Arase,
 * distribuído sob licença MIT e incluído no pacote qrcode-terminal.
 *
 * O arquivo é local para que o PIX continue gerando QR Code mesmo
 * quando uma CDN externa estiver indisponível.
 */
...
const __qrCache = Object.create(null);

function __qrRequire(request) {
  const id = String(request).replace(/^\.\//, "");

  if (__qrCache[id]) {
    return __qrCache[id].exports;
  }

  const factory = __qrFactories[id];

  if (!factory) {
    throw new Error(`Módulo QR não encontrado: ${id}`);
  }

  const module = { exports: {} };
  __qrCache[id] = module;
  factory(__qrRequire, module, module.exports);
  return module.exports;
}

const LocalQRCode = __qrRequire("index");
const QRErrorCorrectLevel = __qrRequire("QRErrorCorrectLevel");
...
export function createQrMatrix(text, level = "M") {
  ...
  const qr = new LocalQRCode(-1, selectedLevel);
...
export function renderQrToCanvas(
  ...
  if (!(canvas instanceof HTMLCanvasElement)) {
    throw new Error("O elemento do QR Code não é válido.");
  }

  const matrix = createQrMatrix(text, level);
  const moduleCount = matrix.length;
  const quietZone = Math.max(2, Number(margin) || 4);
  const cssSize = Math.max(160, Number(size) || 240);
  ...
  const moduleSize = cssSize / totalModules;
  ...
  for (let row = 0; row < moduleCount; row += 1) {
    for (let column = 0; column < moduleCount; column += 1) {
      ...
      const right = Math.ceil(
        (column + quietZone + 1) * moduleSize
      );
      const bottom = Math.ceil(
        (row + quietZone + 1) * moduleSize
      );
